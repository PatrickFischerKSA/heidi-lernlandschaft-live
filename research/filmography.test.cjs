...
vm.runInContext('renderFilms()',c);assert.equal((node('#film-grid').innerHTML.match(/class="film-card"/g)||[]).length,35);assert(node('#film-grid').innerHTML.includes('<video controls'));
...
console.log('35 Datensätze, Quellen/Bilder, alle Kategorien, Suche, Leertreffer, Vergleich und Zusatzregister geprüft.');

const clips=vm.runInContext('filmClips',c);assert.equal(Object.keys(clips).length,30);
node('#film-search').value='';vm.runInContext('renderFilms()',c);
assert.equal((node('#film-grid').innerHTML.match(/data-load-clip=/g)||[]).length,30);
...
assert(node('#app').innerHTML.includes('31 von 35'));
console.log('30 Clip-Player, 4 explizite Lücken, Klickladen, korrekte Anbieter, Zeitbegrenzung und barrierefreie Beschriftung geprüft.');
